...
This is adapted from the scikit-learn example classifying documents
...
print('sentiments added', datetime.now())


processed_features = []

for sentence in reviews_with_beer_info.text:
...
print('text processed', datetime.now())

X_large, X_small, y_large, y_small = train_test_split(processed_features, 
...
def benchmark(clf):
    ...
    f1score = metrics.f1_score(y_test, pred, pos_label='positive')
    f1score_neg = metrics.f1_score(y_test, pred, pos_label='negative')

    print('f1 score positive:   %0.3f' % f1score)
    print('f1 score negative:   %0.3f' % f1score_neg)
    ...
    return clf_descr, f1score_neg, train_time, test_time
